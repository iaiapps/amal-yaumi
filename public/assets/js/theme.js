...
function layout_change_default() {
    ...
    let darkLayout = window.matchMedia("(prefers-color-scheme: dark)").matches
        ? "dark"
        : "light";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    layoutButtons.forEach((button) => {
        button.addEventListener("click", (event) => {
            ...
            const layoutValue =
                target.getAttribute("data-value") === "true" ? "light" : "dark";
...
function preset_change(value) {
    ...
    if (control) {
        const activeElement = document.querySelector(
            ".preset-color > a.active"
        );
        const newActiveElement = document.querySelector(
            `.preset-color > a[data-value='${value}']`
        );
...
function layout_rtl_change(value) {
    ...
    const directionControl = document.querySelector(
        ".theme-direction .btn.active"
    );
    const rtlBtn = document.querySelector(
        ".theme-direction .btn[data-value='true']"
    );
    const ltrBtn = document.querySelector(
...
function layout_change(layout) {
    ...
    const newActiveBtn = document.querySelector(
        `.theme-layout .btn[data-value='${!isDark}']`
    );
    ...
    logoSelectors.forEach((selector) => {
        ...
        if (element) {
            const src = selector.includes("landing-logo")
                ? landingLogoPath
                : logoPaths;
    ...
    if (content && footerWrapper) {
        ...
        if (activeControl) {
            ...
            const newActive = document.querySelector(
                `.theme-container > a[data-value='${value}']`
            );
            if (newActive) {
