    ...
    <div class="card-body">
        <form action="{{ route('guru.mutabaah-item.update', $mutabaah_item) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $mutabaah_item->nama) }}" required>
            ...
            <div class="mb-3">
                ...
                <select name="tipe" class="form-select @error('tipe') is-invalid @enderror" required>
                    ...
                    <option value="ya_tidak" {{ old('tipe', $mutabaah_item->tipe) == 'ya_tidak' ? 'selected' : '' }}>Ya/Tidak</option>
                    <option value="angka" {{ old('tipe', $mutabaah_item->tipe) == 'angka' ? 'selected' : '' }}>Angka</option>
                    <option value="text" {{ old('tipe', $mutabaah_item->tipe) == 'text' ? 'selected' : '' }}>Text</option>
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="urutan" class="form-control @error('urutan') is-invalid @enderror" value="{{ old('urutan', $mutabaah_item->urutan) }}" required>
                @error('urutan')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="mb-3">
                <div class="form-check">
                    <input type="checkbox" name="is_active" class="form-check-input" id="is_active" value="1" {{ old('is_active', $mutabaah_item->is_active) ? 'checked' : '' }}>
