...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{-- <h5>Kelas {{ $role == 'guru' ? 'Bimbingan Saya' : 'Keseluruhan' }}</h5> --}}
            @if ($role == 'guru')
                <div class="d-flex align-items-center justify-content-between">
                    <span class="me-3 badge bg-light text-dark py-2">Limit: {{ $kelas->count() }} / {{ $maxClasses }}
                        Kelas</span>
                    ...
                    <tbody>
                        @forelse($kelas as $k)
                            <tr>
                                ...
                                <td>
                                    <span
                                        class="badge bg-{{ $k->students_count >= $k->kapasitas ? 'danger' : 'success' }}">
                                ...
                                @if ($role == 'guru')
                                    <td>
                                        ...
                                        <form action="{{ route('guru.classroom.destroy', $k) }}" method="POST"
                                            class="d-inline" onsubmit="return confirm('Yakin hapus kelas ini?')">
                                            @csrf
                                            @method('DELETE')
